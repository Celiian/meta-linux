...
module.exports = class CommentManager {
    ...
    /* DATA FORMAT
        {
            ...
            "video":"id de la video sur laquelle le commentaire a été posté",
            'response":""
        }
        */
  ...
  static async mainComment(req, res) {
    ...
    let result = await DatabaseManager.executeQuery(
      `SELECT comment.*, channel.channelName, channel.channelProfilePicture FROM video, comment, commentlist, channel, user WHERE video.id = '${videoId}' AND commentlist.id = video.id AND comment.id = commentlist.commentId AND comment.author = user.id AND user.id = channel.id;`
    );
    if (result.error) {
      console.error("QUERY OR SOMETHING HAS BEEN FUCKED UP");
      res.status(500);
